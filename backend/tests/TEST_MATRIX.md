...
| Document lookup | `test_document_lookup_activates_scope_then_searches_without_opening_canvas` | Covered by document-oriented real-model flows | Document Canvas is covered by FE-E2E-008 and FE-E2E-009B | A filename/topic query activates the document scope, searches names and content, and does not navigate through `open_aina`. |
...
| Managed AINA Project import | `test_aina_projects.py` covers validation, actor isolation, idempotency, persistence, archive integrity, and deletion | Not applicable until a managed runtime exists | FE-E2E-003C | A validated source package remains private and non-callable; the same ID/version cannot be overwritten with different content. |
| Capability failure isolation | `test_tool_failure_is_isolated_and_returned_to_the_model`, `test_agent_resilience.py`, `test_builtin_aina_activation_rejects_arguments_then_allows_model_retry` | Not applicable: failures are injected deterministically | Debug rendering covered by FE-E2E-004 | Every capability validates input before approval/execution; invalid JSON/schema/output, timeout, and invalid AINA responses become tool errors rather than crashing the loop. |
